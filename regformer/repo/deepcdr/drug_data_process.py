...
class DrugDataProcess:
    ...
    def MetadataGenerate(self, Drug_info_file, Cell_line_info_file, Genomic_mutation_file, Drug_feature_file,
        ...
        os.makedirs(f'{self.args.save_dir}/data/', exist_ok=True)
        torch.save([mutation_feature, drug_feature, gexpr_feature, methylation_feature, data_idx], f'{self.args.save_dir}/data/meta.pth')
        print(
    ...
    def FeatureExtract(self, data_idx, drug_feature, mutation_feature, gexpr_feature, methylation_feature):
        ...
        mutation_data_cpu = torch.zeros((nb_instance, 1, nb_mutation_feature, 1), dtype=torch.float32)
        gexpr_data_cpu = torch.zeros((nb_instance, nb_gexpr_features), dtype=torch.float32)
        methylation_data_cpu = torch.zeros((nb_instance, nb_methylation_features), dtype=torch.float32)
        target_cpu = torch.zeros(nb_instance, dtype=torch.float32)
        ...
        for idx in tqdm(range(nb_instance)):
            ...
            mutation_data_cpu[idx, 0, :, 0] = torch.from_numpy(mutation_feature.loc[cell_line_id].values).float()
            gexpr_data_cpu[idx, :] = torch.from_numpy(gexpr_feature.loc[cell_line_id].values).float()
            methylation_data_cpu[idx, :] = torch.from_numpy(methylation_feature.loc[cell_line_id].values).float()
            target_cpu[idx] = ln_IC50

            cancer_type_list.append([cancer_type, cell_line_id, pubchem_id])

        mutation_data = mutation_data_cpu.to(self.device)
        gexpr_data = gexpr_data_cpu.to(self.device)
        methylation_data = methylation_data_cpu.to(self.device)
    ...
    def FeatureExtract(self, data_idx, drug_feature, mutation_feature, gexpr_feature, methylation_feature):
        ...
        drug_data = [None] * nb_instance
        mutation_list = [None] * nb_instance
        gexpr_list = [None] * nb_instance
        methylation_list = [None] * nb_instance
        target_list = [None] * nb_instance
        cancer_type_list = [None] * nb_instance
        ...
        for idx in tqdm(range(nb_instance)):
            ...
            mutation_list[idx] = mutation_feature.loc[cell_line_id].values
            gexpr_list[idx] = gexpr_feature.loc[cell_line_id].values
            methylation_list[idx] = methylation_feature.loc[cell_line_id].values
            target_list[idx] = ln_IC50

            cancer_type_list[idx] = [cancer_type, cell_line_id, pubchem_id]

        mutation_tensor = torch.tensor(np.stack(mutation_list), dtype=torch.float32).unsqueeze(1).unsqueeze(-1).to(self.device)
        gexpr_tensor = torch.tensor(np.stack(gexpr_list), dtype=torch.float32).to(self.device)
        methylation_tensor = torch.tensor(np.stack(methylation_list), dtype=torch.float32).to(self.device)
        target_tensor = torch.tensor(target_list, dtype=torch.float32).to(self.device)

        return drug_data, mutation_tensor, gexpr_tensor, methylation_tensor, target_tensor, cancer_type_list
