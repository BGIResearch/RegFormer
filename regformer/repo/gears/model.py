...
class GEARS_Model(torch.nn.Module):
    ...
    def __init__(self, args):
        ...
        self.gene2ids = args["gene2ids"]
        if self.use_pretrain:
            self.gene_emb = nn.Embedding(self.num_genes, args['layer_size'], max_norm=True)
            self.gene_emb = self.gene_emb.from_pretrained(args['gene_emb_weight'], freeze=False)
            self.liner = nn.Linear(args['layer_size'], hidden_size)
            if self.gene2ids is not None:
                self.ori_gene_ids = [self.gene2ids[i] for i in args['gene_names']]
            self.ori_index = None
        else:
